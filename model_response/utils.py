...
def generate_conversation_summary(conversation_history_list, bedrock_runtime, model_id):
    ...
    conversation_summary_prompt = f"Summarize the following conversation between a user and an AI assistant. The summary should include the context, key points discussed, and any action required. The summary should be concise and informative for a Y-Axis consultant to review:\n\n{conversation_text}"

    try:
        body = {}
        body["anthropic_version"] = "bedrock-2023-05-31"
        body["max_tokens"] = 1000
        body["messages"] = [{"role": "user", "content": conversation_summary_prompt}]
        body[
            "system"
        ] = "You are a helpful assistant. You need to summarize the conversation between a user and an AI assistant. The summary should include the context, key points discussed, and any action required. The summary should be concise and informative."
        ...
        logger.info(
            f"model response parameters for generating summary is {summarization_response}"
        )
        # Parse the response
...
def lead_creation(user_details_dict, salesforce_object, summary):
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            # Update the lead with the conversation summary
            salesforce_object.Lead.update(lead_id, {"Description": summary})
            lead_creation_message = "successfully created"
            return lead_id, lead_creation_message
        except (SalesforceMalformedRequest, SalesforceResourceNotFound) as e:
            logger.info(f"Error creating or updating lead: {e}")
            lead_creation_message = f"Error creating or updating lead: {e}"
            error_message = str(e)

            # Check if the error message contains "Email already exists"
            if "already exists in our database." in error_message.lower():
                return None, lead_creation_message
            ...
            lead_creation_message = f"Unexpected error: {e}"
            return None, lead_creation_message
...
def insert_lead_to_dynamodb(
    ...
    item = {
        ...
        "lead_created_at": time_value,
        "lead_updated_at": time_value,
        "lead_creation_message": lead_creation_message,
    ...
    try:
        ...
        logger.info("Lead Item successfully inserted!")
        return response
    except Exception as e:
        logger.info(f"Error inserting Lead item: {e}")
        return None
